...
- **CI en GitHub Actions** (`.github/workflows/ci.yml`): en cada push/PR a `main` corre las pruebas unitarias de las dos puntas (`npm test`), el build del frontend (`npm run build`) y un chequeo de sintaxis de `backend/src`, `scripts/` y `tests/` (`node --check`, 44 archivos). Las pruebas van **antes** del build a propósito: tardan medio segundo y el build casi un minuto. No bloquea el deploy de Render/Vercel si falla — son procesos independientes, esto solo te avisa.
...
En marcha. **Hecho (2026-09-16)**: 93 pruebas unitarias que corren solas en cada
push (`backend/tests/unit/` y `frontend/tests/unit/`, con `node --test`) — ver
"Cambios recientes". Siguen existiendo los dos recorridos de punta a punta de
