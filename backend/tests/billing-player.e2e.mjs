...
const add = async (b) => (await call(`/player-billing/teams/${TEAM}/members`, { method: 'POST', token: T, body: b })).data.player.id;
const JUAN = await add({ first_name: 'Juan', last_name: 'Perez', group_label: 'U17', monthly_amount: 800, tutor_phone: '52-155-1111-1111' });
const CARLOS = await add({ first_name: 'Carlos', last_name: 'Lopez', group_label: 'U17', monthly_amount: 800 });
const ANA = await add({ first_name: 'Ana', last_name: 'Ramirez', group_label: 'Femenil', monthly_amount: 0, status: 'beca' });
let ov = await overview();
ok(ov.data.players.length === 3, 'el padrón tiene 3');
ok(ov.data.players.every((p) => p.share_token), 'cada uno con su link');
ok(ov.data.players.find((p) => p.player_id === JUAN).tutor_phone === '5215511111111', 'teléfono normalizado');
...
const ch = await call(`/player-billing/teams/${TEAM}/charges`, { method: 'POST', token: T, body: {
  ...
  items: [{ player_id: JUAN, amount: 800 }, { player_id: CARLOS, amount: 800 }, { player_id: ANA, amount: 0 }] } });
...
let led = await call(`/player-billing/teams/${TEAM}/players/${CARLOS}/entries`, { token: T });
...
const tokCarlos = ov.data.players.find((p) => p.player_id === CARLOS).share_token;
await call(`/player-billing/teams/${TEAM}/charges`, { method: 'POST', token: T, body: {
  category: 'uniforme', concept: 'Uniforme', due_date: '2026-10-15', items: [{ player_id: CARLOS, amount: 1500 }] } });
...
const pend = ov.data.pending_payments.find((p) => p.player_id === CARLOS);
...
ov = await overview();
const p2 = ov.data.pending_payments.find((p) => p.player_id === CARLOS);
...
console.log('\n=== 8. Aislamiento ===');
const tokJuan = ov.data.players.find((p) => p.player_id === JUAN).share_token;
